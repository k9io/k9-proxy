...
func Write_Cache(cacheFile string, body string) {
	...
	if err_stat != nil {
		...
		err_write := os.WriteFile(cacheFile, []byte(body), 0600)
	...
	current_cache, _ := Read_Cache(cacheFile)
	...
	if bytes.Equal([]byte(current_cache[:]), []byte(body[:])) == false {
		...
		err := os.WriteFile(cacheFile, []byte(body), 0600)
